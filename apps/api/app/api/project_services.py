...
async def get_project_services(project_id: str, db: Session = Depends(get_db)):
    ...
    connections = (
        ...
        .filter(ProjectServiceConnection.project_id == project_id)
        .all()
    )
...
async def disconnect_service(
    project_id: str, provider: str, db: Session = Depends(get_db)
):
    ...
    if not connection:
        raise HTTPException(
            status_code=404, detail=f"{provider.capitalize()} service not connected"
        )
