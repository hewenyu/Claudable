...
from app.services.vercel_service import (
    VercelAPIError,
    VercelService,
...
async def check_vercel_project_availability(
    project_name: str, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if result["exists"]:
            raise HTTPException(
                status_code=409, detail=f"Project '{project_name}' already exists"
            )
        ...
        raise HTTPException(
            status_code=500, detail="Failed to check project availability"
        )


@router.post(
    "/projects/{project_id}/vercel/connect", response_model=VercelConnectResponse
)
async def connect_vercel_project(
    project_id: str, request: VercelConnectRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        if not project_result.get("success"):
            raise HTTPException(
                status_code=500, detail="Failed to create Vercel project"
            )
        ...
        canonical_project_name = project_result.get(
            "project_name", request.project_name
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to connect Vercel project: {str(e)}"
        )


@router.post(
    "/projects/{project_id}/vercel/deploy", response_model=VercelDeploymentResponse
)
async def deploy_to_vercel(
    project_id: str, request: VercelDeploymentRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        try:
            ...
            vercel_data["last_deployment_url"] = (
                f"https://{deployment_result['deployment_url']}"
                if not str(deployment_result["deployment_url"]).startswith("http")
                else deployment_result["deployment_url"]
            )
        ...
        try:
            ...
            logger.info(
                f"🚀 Starting background monitoring for deployment {deployment_result['deployment_id']}"
            )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to deploy to Vercel: {str(e)}"
        )
...
async def get_vercel_connection_status(project_id: str, db: Session = Depends(get_db)):
    ...
    project_connected = bool(
        ...
        and connection.service_data
        and (
            connection.service_data.get("project_id")
            or connection.service_data.get("project_name")
        )
    )
    ...
    return {
        ...
        and token_exists,  # Both token and project must exist
