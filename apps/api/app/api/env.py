...
async def get_env_vars(project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db_env_vars = db.query(EnvVar).filter(EnvVar.project_id == project_id).all()
...
async def create_env_variable(
    project_id: str, body: EnvVarCreate, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to create env var: {str(e)}"
        )


@router.put("/{project_id}/{key}")
async def update_env_variable(
    project_id: str, key: str, body: EnvVarUpdate, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to update env var: {str(e)}"
        )
...
async def delete_env_variable(project_id: str, key: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to delete env var: {str(e)}"
        )
...
async def get_sync_conflicts(project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return ConflictResponse(conflicts=conflicts, has_conflicts=len(conflicts) > 0)
...
async def sync_file_to_database(project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sync file to DB: {str(e)}"
        )
...
async def sync_database_to_file(project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to sync DB to file: {str(e)}"
        )
...
async def upsert_env(
    ...
    existing = (
        db.query(EnvVar)
        .filter(EnvVar.project_id == project_id, EnvVar.key == body.key)
        .first()
    )

    if existing:
        # Update existing
        return await update_env_variable(
            project_id, body.key, EnvVarUpdate(value=body.value), db
        )
    else:
        # Create new
