...
# CLI 옵션과 체크 명령어 정의
CLI_OPTIONS = [
    {"id": "claude", "name": "Claude Code", "check_command": ["claude", "--version"]},
    {
        "id": "cursor",
        "name": "Cursor Agent",
...
async def check_cli_installation(cli_id: str, command: list) -> CLIStatusResponse:
    ...
    try:
        ...
        if process.returncode == 0:
            ...
            return CLIStatusResponse(cli_id=cli_id, installed=True, version=version)
        else:
            ...
            return CLIStatusResponse(cli_id=cli_id, installed=False, error=error_msg)

    except FileNotFoundError:
        # 명령어를 찾을 수 없는 경우 (설치되지 않음)
        return CLIStatusResponse(
            cli_id=cli_id, installed=False, error="Command not found"
        )
    except Exception as e:
        # 기타 예외
        return CLIStatusResponse(cli_id=cli_id, installed=False, error=str(e))
...
async def get_cli_status() -> Dict[str, Any]:
    ...
    for cli_id, status in cli_results:
        results[cli_id] = {
            ...
            "version": (
                status.get("models", ["Unknown"])[0] if status.get("models") else None
            ),
