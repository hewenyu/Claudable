...
def _safe_join(repo_root: str, rel_path: str) -> str:
    ...
    if not full.startswith(os.path.normpath(repo_root) + os.sep) and os.path.normpath(
        full
...
async def repo_tree(
    project_id: str, dir: str = Query("."), db: Session = Depends(get_db)
) -> List[RepoEntry]:
    ...
    for child in sorted(
        Path(target).iterdir(), key=lambda p: (p.is_file(), p.name.lower())
    ):
