...
async def start_preview(
    ...
    # Start preview
    process_name, port = start_preview_process(
        project_id, project.repo_path, port=body.port
    )
    result = {
        "success": True,
        "port": port,
    ...
    if not result["success"]:
        raise HTTPException(
            status_code=500, detail=result.get("error", "Failed to start preview")
        )
...
async def get_all_error_logs(project_id: str, db: Session = Depends(get_db)):
...
async def get_preview_logs_endpoint(
    project_id: str, lines: int = 100, db: Session = Depends(get_db)
):
    ...
    return PreviewLogsResponse(logs=logs, running=status.get("running", False))
...
async def restart_preview(
    ...
    if not result["success"]:
        raise HTTPException(
            status_code=500, detail=result.get("error", "Failed to restart preview")
        )
