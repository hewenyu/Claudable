...
async def initialize_project_background(
    project_id: str, project_name: str, body: ProjectCreate
):
...
async def install_project_dependencies(
    ...
    background_tasks.add_task(
        install_dependencies_background, project_id, project.repo_path
    )
...
async def list_projects(db: Session = Depends(get_db)) -> List[Project]:
    ...
    last_message_subquery = (
        db.query(
            Message.project_id, func.max(Message.created_at).label("last_message_at")
        )
    ...
    projects_with_last_message = (
        ...
        .outerjoin(
            last_message_subquery, ProjectModel.id == last_message_subquery.c.project_id
        )
    ...
    for project, last_message_at in projects_with_last_message:
        ...
        service_connections = (
            ...
            .filter(ProjectServiceConnection.project_id == project.id)
            .all()
        )

        for conn in service_connections:
            services[conn.provider] = {"connected": True, "status": conn.status}

        # Ensure all service types are represented
        for provider in ["github", "supabase", "vercel"]:
            if provider not in services:
                services[provider] = {"connected": False, "status": "disconnected"}
...
async def create_project(body: ProjectCreate, db: Session = Depends(get_db)) -> Project:
    ...
    if not selected_model:
        if preferred_cli == "claude":
            ...
            selected_model = "sonnet-4"  # Use unified model name
    fallback_enabled = (
        body.fallback_enabled if body.fallback_enabled is not None else True
    )
