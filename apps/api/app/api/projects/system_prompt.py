...
from app.services.claude_act import get_system_prompt, load_system_prompt
...
async def get_project_system_prompt(project_id: str, db: Session = Depends(get_db)):
    ...
    return SystemPromptResponse(system_prompt=system_prompt, project_id=project_id)


@router.put("/{project_id}/system-prompt")
async def update_project_system_prompt(
    project_id: str, body: SystemPromptUpdate, db: Session = Depends(get_db)
):
    ...
    return {"message": "System prompt updated successfully", "project_id": project_id}
...
async def reset_project_system_prompt(project_id: str, db: Session = Depends(get_db)):
    ...
    return {"message": "System prompt reset to default", "project_id": project_id}
