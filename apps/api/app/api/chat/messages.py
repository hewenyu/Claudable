...
async def get_active_session(project_id: str, db: Session = Depends(get_db)):
    ...
    active_session = (
        ...
        )  # Include both active and running
    ...
    return {
        ...
        "started_at": (
            active_session.started_at.isoformat() if active_session.started_at else None
        ),
    }
...
async def send_message(
    project_id: str, body: SendMessageRequest, db: Session = Depends(get_db)
):
...
async def clear_messages(
    ...
    await manager.send_message(
        project_id, {"type": "messages_cleared", "conversation_id": conversation_id}
    )
...
async def get_active_requests(project_id: str, db: Session = Depends(get_db)):
    ...
    active_count = (
        ...
        .filter(UserRequest.is_completed.is_(False))
        .count()
    )
