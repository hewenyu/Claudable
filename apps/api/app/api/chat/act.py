...
async def execute_act_instruction(
    ...
    try:
        ...
        project_info = {
            ...
            "fallback_enabled": (
                project.fallback_enabled
                if project.fallback_enabled is not None
                else True
            ),
    ...
    try:
        ...
        if cli_preference is None:
            try:
                ...
                ui.warning(
                    f"Unknown CLI type '{project_preferred_cli}', falling back to Claude",
                    "CHAT",
                )
                cli_preference = CLIType.CLAUDE

        ui.info(
            f"Using {cli_preference.value} with {project_selected_model or 'default model'}",
            "CHAT",
        )
        ...
        if result and result.get("success"):
            ...
            error_msg = Message(
                ...
                message_type="error",
                content=(
                    result.get("error", "Failed to execute chat instruction")
                    if result
                    else "No CLI available"
                ),
                metadata_json={
                    "type": "chat_error",
        ...
        await manager.broadcast_to_project(
            ...
            {
                "type": "chat_complete",
                "data": {"status": session.status, "session_id": session.id},
            },
        )
        ...
        await manager.broadcast_to_project(
            ...
            {
                "type": "chat_complete",
                "data": {"status": "failed", "session_id": session.id, "error": str(e)},
            },
        )
...
async def execute_act_task(
    ...
    try:
        ...
        if request_id:
            user_request = (
                db.query(UserRequest).filter(UserRequest.id == request_id).first()
            )
            if user_request:
        ...
        if result and result.get("success"):
            ...
            if result.get("has_changes"):
                try:
                    commit_message = (
                        f"🤖 {result.get('cli_used', 'AI')}: {instruction[:100]}"
                    )
                        ...
                        await manager.send_message(
                            project_id,
                            {
                                ...
                                "data": {
                                    "commit_hash": commit_result["commit_hash"],
                                    "message": commit_message,
                                    "files_changed": commit_result.get(
                                        "files_changed", 0
                                    ),
                                },
                            },
            ...
            if request_id:
                ...
                if user_request:
                    ...
                    ui.success(
                        f"UserRequest {request_id[:8]}... marked as completed", "ACT"
                    )
                else:
                    ui.warning(
                        f"UserRequest {request_id[:8]}... not found for completion",
                        "ACT",
                    )
            ...
            error_msg = Message(
                ...
                message_type="error",
                content=(
                    result.get("error", "Failed to execute instruction")
                    if result
                    else "No CLI available"
                ),
                metadata_json={
            ...
            if request_id:
                ...
                if user_request:
                    ...
                    user_request.error_message = (
                        result.get("error") if result else "No CLI available"
                    )
                    ui.warning(
                        f"UserRequest {request_id[:8]}... marked as failed", "ACT"
                    )
                else:
                    ui.warning(
                        f"UserRequest {request_id[:8]}... not found for failure marking",
                        "ACT",
                    )
        ...
        try:
            db.commit()
            ui.success(
                f"Database commit successful for request {request_id[:8] if request_id else 'unknown'}...",
                "ACT",
            )
...
async def run_act(
    ...
    if image_paths:
        image_refs = [
            f"Image #{i+1} path: {path}" for i, path in enumerate(image_paths)
        ]
    ...
    project_info = {
        ...
        "preferred_cli": project.preferred_cli or "claude",
        "fallback_enabled": (
            project.fallback_enabled if project.fallback_enabled is not None else True
        ),
        "selected_model": project.selected_model,
    }
