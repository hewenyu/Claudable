...
async def get_cli_preference(project_id: str, db: Session = Depends(get_db)):
    ...
    return {"preferred_cli": preferred_cli, "selected_model": selected_model}


@router.post("/{project_id}/cli-preference")
async def set_cli_preference(
    project_id: str, body: CLIPreferenceRequest, db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Invalid CLI type: {body.preferred_cli}"
        )
...
async def set_model_preference(
    project_id: str, body: ModelPreferenceRequest, db: Session = Depends(get_db)
):
...
async def get_cli_status(project_id: str, cli_type: str, db: Session = Depends(get_db)):
