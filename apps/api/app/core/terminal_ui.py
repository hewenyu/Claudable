...
class TerminalUI:
    ...
    def panel(self, content: str, title: Optional[str] = None, style: str = "blue"):
        ...
        panel = Panel(
            content, title=title, border_style=style, box=box.ROUNDED, padding=(1, 2)
        )
    ...
    def connection_status(self, project_id: str, status: str):
        ...
        self.log(
            f"WebSocket {status} for project: {project_id}", LogLevel.INFO, "WebSocket"
        )

    def session_info(self, session_id: str, cli_type: str, model: str):
        """CLI session information"""
        self.log(
            f"Session {session_id[:8]}... started with {cli_type} using {model}",
            LogLevel.INFO,
            "Session",
        )

    def operation_result(
        self, operation: str, success: bool, details: Optional[str] = None
    ):
