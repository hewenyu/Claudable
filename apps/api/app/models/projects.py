...
class Project(Base):
    ...
    active_cursor_session_id: Mapped[str | None] = mapped_column(
        String(128), nullable=True
    )  # Cursor Agent session ID

    # CLI Preferences
    preferred_cli: Mapped[str] = mapped_column(
        String(32), default="claude", nullable=False
    )  # claude, cursor
    selected_model: Mapped[str | None] = mapped_column(
        String(64), nullable=True
    )  # Selected model for the CLI
    fallback_enabled: Mapped[bool] = mapped_column(
        default=True, nullable=False
    )  # Enable fallback to other CLIs
    ...
    tools_usage = relationship(
        "ToolUsage", back_populates="project", cascade="all, delete-orphan"
    )
    commits = relationship(
        "Commit", back_populates="project", cascade="all, delete-orphan"
    )
    env_vars = relationship(
        "EnvVar", back_populates="project", cascade="all, delete-orphan"
    )
    ...
    user_requests = relationship(
        "UserRequest", back_populates="project", cascade="all, delete-orphan"
    )
