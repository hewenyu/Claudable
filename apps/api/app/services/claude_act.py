...
def find_prompt_file() -> Path:
    ...
    fallback_locations = [
        ...
        / "docs"
        / "system-prompt.md",  # project-root/docs/
    ...
    try:
        ...
        if prompt_file.exists():
            with open(prompt_file, "r", encoding="utf-8") as f:
                ...
                print(
                    f"✅ Loaded system prompt from: {prompt_file} ({len(content)} chars)"
                )
...
async def generate_diff_with_logging(
    ...
    effective_system_prompt = (
        system_prompt if system_prompt is not None else get_system_prompt()
    )
    ...
    try:
        ...
        if log_callback:
            await log_callback(
                "text", {"content": "🚀 Starting Claude Code execution..."}
            )
                ...
                for block in message.content:
                    if isinstance(block, TextBlock):
                        ...
                        if log_callback:
                            ...
                            diff_info = None
                            if (
                                tool_info.get("name") in ["Edit", "MultiEdit"]
                                and block.content
                            ):
                                try:
                                    content_str = str(block.content)
                                    if (
                                        "updated" in content_str.lower()
    ...
    if message_count == 0:
        print(
            "No messages received from Claude Code SDK - falling back to simple response"
        )
    ...
    if "<COMMIT_MSG>" in response_text and "</COMMIT_MSG>" in response_text:
        commit_msg = (
            response_text.split("<COMMIT_MSG>", 1)[1]
            .split("</COMMIT_MSG>", 1)[0]
            .strip()
        )
    ...
    if "<SUMMARY>" in response_text and "</SUMMARY>" in response_text:
        diff_summary = (
            response_text.split("<SUMMARY>", 1)[1].split("</SUMMARY>", 1)[0].strip()
        )
