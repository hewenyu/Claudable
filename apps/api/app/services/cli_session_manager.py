...
class CLISessionManager:
    ...
    def set_session_id(
        self, project_id: str, cli_type: CLIType, session_id: str
    ) -> bool:
        ...
        ui.success(
            f"Set {cli_type.value} session ID for project {project_id}: {session_id}",
            "Session",
        )
        return True
    ...
    def get_session_stats(self, project_id: str) -> Dict[str, Any]:
        ...
        for stat in session_stats:
            stats[stat.cli_type] = {
                ...
                "avg_duration_ms": (
                    int(stat.avg_duration_ms) if stat.avg_duration_ms else 0
                ),
                ...
                "active_session_id": self.get_session_id(
                    project_id, CLIType(stat.cli_type)
                ),
            }
    ...
    def set_preferred_cli(
        self, project_id: str, cli_type: CLIType, fallback_enabled: bool = True
    ) -> bool:
        ...
        print(
            f"✅ [Session] Set preferred CLI for project {project_id}: {cli_type.value} (fallback: {fallback_enabled})"
        )
        return True
    ...
    def migrate_legacy_sessions(self, project_id: str) -> Dict[str, int]:
        ...
        updated_sessions = (
            ...
            .filter(
                ChatSession.project_id == project_id, ChatSession.cli_type.is_(None)
            )
        ...
        messages = (
            ...
            .filter(
                ...
                Message.metadata_json.is_not(None),
            )
            .all()
        )
    ...
    def cleanup_stale_sessions(self, project_id: str, days_threshold: int = 30) -> int:
        ...
        ui.info(
            f"Project {project_id}: Cleared {cleared_count} stale session IDs",
            "Cleanup",
        )
