...
class VercelService:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{VERCEL_API_BASE}/v2/user", headers=self.headers
                ) as response:
                ...
                async with session.post(
                    ...
                    if response.status == 200 or response.status == 201:
                        ...
                        raise VercelAPIError(
                            f"Failed to create project: {error_msg}", response.status
                        )
    ...
    async def get_project(self, project_id: str) -> Dict[str, Any]:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"{VERCEL_API_BASE}/v9/projects/{project_id}", headers=self.headers
                ) as response:
                    if response.status == 200:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if response.status == 200 or response.status == 201:
                        ...
                        raise VercelAPIError(
                            f"Failed to create deployment: {error_msg}", response.status
                        )
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    ...
                    if response.status == 200:
                        ...
                        final_url = (
                            deployment.get("aliasFinal")
                            or (
                                deployment.get("alias")[0]
                                if deployment.get("alias")
                                else None
                            )
...
async def check_project_availability(
    access_token: str, project_name: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{VERCEL_API_BASE}/v10/projects", headers=service.headers
            ) as response:
                if response.status == 200:
...
async def start_deployment_monitoring(
    project_id: str, deployment_id: str, vercel_token: str, db_session_factory
) -> None:
    ...
    task = asyncio.create_task(
        monitor_deployment_status(
            project_id, deployment_id, vercel_token, db_session_factory
        )
    )
    active_monitoring_tasks[project_id] = task

    logger.info(
        f"🚀 Started deployment monitoring for project {project_id}, deployment {deployment_id}"
    )
        ...
        while True:
            try:
                ...
                if elapsed > max_duration_minutes:
                    logger.warning(
                        f"⏰ Deployment {deployment_id} monitoring timed out after {max_duration_minutes} minutes"
                    )
                    break
                ...
                if (
                    status_data["status"] in ["READY", "ERROR"] or elapsed < 0.1
                ):  # 처음 또는 완료 시에만
                    logger.info(
                        f"🔍 Checking deployment {deployment_id} status... (elapsed: {elapsed:.1f}min)"
                    )
                    logger.info(
                        f"🔍 Got status: {status_data['status']} for deployment {deployment_id}"
                    )
                ...
                if status_data["status"] == "READY":
                    ...
                    logger.info(
                        f"🎉 READY response - aliasFinal: {raw_response.get('aliasFinal')}, alias: {raw_response.get('alias', [])[:2]}, url: {raw_response.get('url')}"
                    )
                    logger.info(f"🎉 Final URL selected: {status_data['url']}")

                # DB 업데이트
                await update_deployment_status_in_db(
                    project_id, status_data, db_session_factory
                )
                ...
                if is_ready or is_error:
                    logger.info(
                        f"✅ Deployment {deployment_id} finished with status: {status_data['status']}"
                    )
                    break
...
async def update_deployment_status_in_db(
    project_id: str, status_data: Dict[str, Any], db_session_factory
) -> None:
    """DB의 배포 상태 업데이트"""
    ...
    try:
        ...
        try:
            ...
            if connection:
                service_data = (
                    dict(connection.service_data) if connection.service_data else {}
                )
                ...
                if status_data["status"] == "READY":
                    service_data["deployment_url"] = (
                        f"https://{status_data['url']}"
                        if not str(status_data["url"]).startswith("http")
                        else status_data["url"]
                    )
                    service_data["last_deployment_at"] = (
                ...
                if "current_deployment" in updated_data:
                    if status_data["status"] == "READY":
                        logger.info(
                            f"✅ Successfully saved READY deployment to DB for project {project_id}"
                        )
                        logger.info(
                            f"📝 Final service_data keys: {list(updated_data.keys())}"
                        )
                else:
                    logger.error(
                        f"❌ DB update verification failed for project {project_id}"
                    )
            else:
