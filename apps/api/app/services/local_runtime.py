...
def _monitor_preview_errors(project_id: str, process: subprocess.Popen):
    ...
    success_patterns = ["✓ Ready in", "○ Compiling", "✓ Compiled", "✓ Starting"]
    ...
    def collect_error_context(line_text):
        ...
        if current_error and (
            ...
            if (
                len(error_lines) > 15
            ):  # 런타임 에러는 스택트레이스가 길 수 있으므로 15라인까지
    ...
    def send_error_with_context(error_id, lines):
        ...
        try:
            loop = asyncio.new_event_loop()
            asyncio.set_event_loop(loop)
            loop.run_until_complete(manager.send_message(project_id, message_data))
...
def _should_install_dependencies(repo_path: str) -> bool:
    ...
    print(
        f"Package files changed, will install dependencies (new hash: {final_hash[:8]}...)"
    )
    return True
...
def start_preview_process(
    project_id: str, repo_path: str, port: Optional[int] = None
) -> tuple[str, int]:
    """
    ...
    try:
        ...
        try:
            ...
            if (
                os.path.exists(pnpm_lock)
                or os.path.exists(yarn_lock)
                or os.path.isdir(pnpm_dir)
            ):
                ...
                try:
                    ...
                    shutil.rmtree(
                        os.path.join(repo_path, "node_modules"), ignore_errors=True
                    )
        ...
        if _should_install_dependencies(repo_path):
            ...
            print(
                f"Dependencies installed successfully for project {project_id} using npm"
            )
        else:
            print(
                f"Dependencies already up to date for project {project_id}, skipping npm install"
            )
        ...
        error_thread = threading.Thread(
            target=_monitor_preview_errors, args=(project_id, process), daemon=True
        )
        error_thread.start()
        ...
        print(
            f"Next.js dev server started for {project_id} on port {port} (PID: {process.pid})"
        )
...
def stop_preview_process(project_id: str, cleanup_cache: bool = False) -> None:
    ...
    if process:
        try:
            ...
            print(
                f"Stopped Next.js dev server for project {project_id} (PID: {process.pid})"
            )
